...
public final class TickHandler {
    ...
    public void onUnloadChunk(ChunkEvent.Unload event) {
        ...
        this.blockEntities.removeChunk(level, ChunkPos.asLong(chunkPos.x, chunkPos.z));
    }
    ...
    private void readyBlockEntities(World world) {
        ...
        for (long packedChunkPos : workSet) {
            BlockPos checkPos = new BlockPos((ChunkPos.getX(packedChunkPos) << 4), 0, (ChunkPos.getZ(packedChunkPos) << 4));
            if (Platform.areBlockEntitiesTicking(world, checkPos)) {
                ...
                if (chunkQueue == null) {
                    AELog.warn("Chunk %s was unloaded while we were readying block entities",
                        new ChunkPos(ChunkPos.getX(packedChunkPos), ChunkPos.getZ(packedChunkPos)));
                    continue;
                }
