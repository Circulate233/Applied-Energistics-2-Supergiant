...
public class ThresholdExportBusPart extends PreciseExportBusPart implements ThresholdModeHost {
    ...
    private boolean passesThreshold(GenericStack stack, appeng.api.networking.storage.IStorageService storageService) {
        long stored = storageService.getCachedInventory().get(stack.what());
        return (this.mode == ThresholdMode.GREATER) == (stored > stack.amount());
    }

    private long getMaxOutput(GenericStack stack, appeng.api.networking.storage.IStorageService storageService) {
    ...
    @Override
    protected appeng.container.GuiIds.GuiKey getGuiKey() {
        return appeng.container.GuiIds.GuiKey.THRESHOLD_EXPORT_BUS;
    }
