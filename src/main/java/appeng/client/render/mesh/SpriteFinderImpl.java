...
public class SpriteFinderImpl implements SpriteFinder {
    private static final Map<TextureMap, CacheEntry> CACHE = new WeakHashMap<>();

    private final Node root;
    private final TextureMap spriteAtlasTexture;

    public SpriteFinderImpl(Iterable<TextureAtlasSprite> sprites, TextureMap spriteAtlasTexture) {
    ...
    public static SpriteFinderImpl get(TextureMap atlas) {
        Collection<TextureAtlasSprite> sprites = atlas.mapUploadedSprites.values();
        synchronized (CACHE) {
            CacheEntry entry = CACHE.get(atlas);
            if (entry == null || entry.sprites != sprites) {
                entry = new CacheEntry(sprites, new SpriteFinderImpl(sprites, atlas));
                CACHE.put(atlas, entry);
            }
            return entry.finder;
        }
    }
    ...
    private record CacheEntry(Collection<TextureAtlasSprite> sprites, SpriteFinderImpl finder) {
    }
