...
public abstract class GeometryHelper {
     ...
     */
    public static boolean isQuadCubic(@NotNull EnumFacing lightFace, QuadView quad) {
        if (lightFace == null) {
            return false;
        }
    ...
    public static EnumFacing lightFace(QuadView quad) {
        ...
        return switch (GeometryHelper.longestAxis(normal)) {
            case X -> normal.x > 0 ? EnumFacing.EAST : EnumFacing.WEST;
            case Y -> normal.y > 0 ? EnumFacing.UP : EnumFacing.DOWN;
            case Z -> normal.z > 0 ? EnumFacing.SOUTH : EnumFacing.NORTH;
        };
    }
